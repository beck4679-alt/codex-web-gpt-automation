...
function fixture({ enabled = false, temporary = true, ambiguous = false } = {}) {
  let opened = false;
  let clicks = 0;
  ...
  const trigger = { ...base, getAttribute: () => enabled ? 'Personalized' : 'Unpersonalized', click: () => { opened = true; clicks++; } };
  ...
  const yes = row('Personalized', () => enabled, () => { enabled = true; opened = false; clicks++; });
  const no = row('Unpersonalized', () => !enabled, () => { throw Error('must not disable'); });
